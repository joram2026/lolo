    ...
    <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-3 sm:p-4 bg-slate-950/80 backdrop-blur-sm animate-fade-in">
      <div 
        className={`w-full max-w-xl rounded-3xl border shadow-2xl overflow-hidden transition-all duration-300 ${
          isLightTheme 
            ? 'bg-[#FFFDF8] border-amber-200/90 text-zinc-900 shadow-amber-900/10' 
            : 'bg-slate-900 border-slate-800 text-white shadow-black/40'
        }`}
      >
        {/* Header Bar */}
        <div className={`px-5 py-4 border-b flex items-center justify-between gap-3 ${
          isLightTheme ? 'bg-[#FFF8EC]/90 border-amber-200/80' : 'bg-slate-950/80 border-slate-800'
        }`}>
        ...
        <div className="p-4 sm:p-6 space-y-4 max-h-[80vh] overflow-y-auto">
          ...
          <div className={`p-4 rounded-2xl border ${
            ...
            <div className="grid grid-cols-1 sm:grid-cols-[1fr,auto,1fr] items-center gap-3">
              ...
              <div className={`p-3 rounded-xl border space-y-1.5 ${
                isLightTheme ? 'bg-[#FFF8EC]/60 border-amber-200/70' : 'bg-slate-900/80 border-slate-800'
              }`}>
          ...
          <div className={`p-3.5 rounded-2xl border text-xs space-y-1.5 ${
            isLightTheme ? 'bg-[#FFF8EC]/80 border-amber-200/90 text-zinc-800' : 'bg-slate-950/60 border-slate-800 text-zinc-300'
          }`}>
        ...
        <div className={`p-4 border-t flex gap-2.5 ${
          isLightTheme ? 'bg-[#FFF8EC]/60 border-amber-200/80' : 'bg-slate-950 border-slate-800'
        }`}>
          <button
            type="button"
            onClick={onClose}
